...
public class DataActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		for (Person person : persons) {
		 ...
		 /**
		  * 定义一个适配器 参数一 上下文信息 当前的上下文信息 是当前的类 参数二 加载的值 参数三 加载的视图界面文件 参数四 加载的目录
		  * 这个目录 是根据键值去取的值 在上面已经设置好了这个键值对 参数五 加载的数据对应的属性
		  */
		...
		listview.setOnItemClickListener(new OnItemClickListener() {
			 /**
			  * 参数一 表示 点击的 listview 参数二 表示点击的最外层的那个元素 说明 int position, long id
			  * 是所在行的id
			  */
			@Override
